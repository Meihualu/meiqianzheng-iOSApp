...
+(void)addCommodity:(CommodityModel *)item{
    ...
    NSInteger categoryId = [ShoppingCarViewModel queryCategoryTypeWithCategoryName:item.category];
    NSLog(@"categoryId = %zd\n",categoryId);
    if (categoryId == -1) {
        [_db executeUpdateWithFormat:@"INSERT INTO t_category (category) VALUES (%@);",item.category];
        categoryId = [ShoppingCarViewModel queryCategoryTypeWithCategoryName:item.category];
    }
    
    NSLog(@"商品的categoryId = %zd\n",categoryId);
    [_db executeUpdateWithFormat:@"INSERT INTO t_commodity (barcode,name,unit,categoryid,category,subcategory,price,discountype) VALUES (%@,%@,%@,%ld,%@,%@,%f,%@);",item.barcode,item.name,item.unit,(long)categoryId,item.category,item.subCategory,item.price,item.promotionType[0]];
}
